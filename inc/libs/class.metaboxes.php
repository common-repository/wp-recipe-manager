...
class TMU_Meta_Box {
	...
	public function __construct() {
		...
		define( 'TMU_META_URL', plugin_dir_url( __FILE__ ) );
	...
	public function admin_assets() {
		wp_enqueue_style( 'tmu-meta-backend', TMU_META_URL . 'assets/css/backend.css' );
		wp_enqueue_script( 'jquery.repeater.min', TMU_META_URL . 'assets/js/jquery.repeater.min.js', array( 'jquery' ) );
		wp_enqueue_script( 'tmu-meta-backend', TMU_META_URL . 'assets/js/backend.js', array(
			'jquery.repeater.min',
			'jquery-ui-sortable',
			'jquery'
		) );
	...
	public function init() {
		...
		$this->meta_boxes = wp_parse_args( apply_filters( 'tmu_meta_boxes', array() ) );
		if ( $this->meta_boxes ) {
	...
	public function reader_field( $meta_value, $field ) {
		...
		switch ( $field['type'] ) {
			...
			case 'textarea':
				...
				break;
			case 'wpeditor':
				wp_editor( $value, uniqid( 'tmu' ), array( 'textarea_name' => $name, 'textarea_rows' => 5 ) );
				break;
			default:
...
if ( ! function_exists( 'tmu_meta' ) ) {
	function tmu_meta( $meta_key, $single = false, $post_id = null ) {
